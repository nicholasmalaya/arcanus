...
%%
%%    min  1/2 * ||u - z||^2 + gamma * ||grad a dot grad a + tau||^0.5,
%%     a
%%         where u is the solution of
%%
%%              - div (a * grad u) + v dot grad u = f     on Omega
%%                                              u = 0     on bdry(Omega)
%%
...
fem.equ.expr.gamma = '5e-10';
fem.equ.expr.tau = '0.1';
datanoise = 0.01;
...
fem.equ.expr.goal = '-(atrue * (udx * udx_test + udy * udy_test) + ud_test * (vx * udx + vy * udy) - f * ud_test)';
fem.equ.expr.state = '-(a * (ux * ux_test + uy * uy_test) + u_test * (vx * ux + vy * uy) - f * u_test)';
fem.equ.expr.adjoint = '-(a * (px * px_test + py * py_test) - (ud - u) * p_test + p * (vx * px_test + vy * py_test))';
fem.equ.expr.control = ['(grad * grad_test - 0.5 * gamma * (ax * gradx_test + ay * grady_test) * (ax*ax+ay*ay+tau)^(-0.5)'...
