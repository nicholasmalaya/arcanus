...
\begin{align}
  ...
  \label{first}
  \frac{\partial^2 \bar z}{\partial y^2} &= \frac{\bar z_{j+1}-2\bar z_{j}+z_{j-1}}{\Delta y^2} 
\end{align}
...
we could calculate it directly. However, do don't know P() (the PDF). Instead, we will use 
what Peter's calls the ``Presumed Shape PDF Approach''. We must pick a probability distribution. 
