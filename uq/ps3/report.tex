...
300,000 long, and downsampled to 10,000 uncorrelated samples for the
integrals. We then confirmed that these samples were uncorrelated using
arsel on the downsampled chains. Later, we found that these 10k uncorrelated samples were
complete overkill with respect to the integration. Only a few hundred
samples were needed to compute the posterior distributions shown below,
wich are generally quite smooth (and no further smoothing function or
KDE was utilized afterwards). 
