...
\centering
 ...
 \caption{Iso-contours of the inner thermal core
...
%The entrainment of fluid is
%limited to a region immediately surrounding the vanes, limiting the
...
would create a larger vertical momentum flux and consequently a larger
kinetic energy flux.  
...
Regardless, these slices lend credibility to the 
...
described in Section~\ref{sec:bc}. The turning vanes are drawn in
...
the forcing field. The radial region of forcing is between $\{0.96-3.4\}$ meters 
...
the base flow temperature. The iso-contour of a three Kelvin threshold
...
\begin{figure}[htb]
 ...
 ``two-cell'' structure akin to that observed in the naturally occurring
...
where the device is entirely driven by kinetic energy from ambient winds which drive 
...
aggregate only accounts for a modest portion of the flux, so greatly
increase the total? 

Measuring the energy flux entering the front face of the vanes for the
