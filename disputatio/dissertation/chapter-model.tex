...
Even in ``ideal'' situations, the theory has seldom been found to be
accurate to more than 10\%\cite{?}. Nevertheless, for our case, with
minimal surface roughness and our interest constrained to the near
surface layer, these functions are applicable and reasonably
...
is applied in the annular region that contains the vanes.\todo{add image} Vane
...
Just an actuator disk or BEM model. 
Practical utility-scale wind turbines achieve at peak 75\% to 80\% of
the Betz limit. However, betz is likely not accurate here.

The normal in the blade's velocity direction is,\todo{add image} 
\begin{equation*}
n_B = \frac{u_B}{||u_B||}. 
\end{equation*}
...
These parameters, $\beta$, etc. are visually depicted in Figure
\ref{fig:turbine_image}.  
