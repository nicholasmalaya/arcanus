...
vertical mixing in the atmosphere.\todo{this entire section needs
another read} 
...
{u^*}^{3}$. The non-dimensional scale, $\xi$, holds a
$(u^*)^{-3}$ scaling through the dependence on the M-O length, 
$L_{M-O}$, in the denominator. 
Therefore, the overall function will not depend on $u^*$ only when
the function $\varphi$ scales to the $-\frac{4}{3}$ power. 
