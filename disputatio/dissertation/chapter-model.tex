...
vertical mixing in the atmosphere.\todo{this entire section still needs to be fixed} 
...
velocity); the heat flux to the ground, $q$; and the specific heat. %\todo{is this right?}
...
\begin{eqnarray}
 ...
 \textbf{Specific Heat:}& \enskip c_p \enskip \dot = \enskip [m]^2 [2]^{-2} [K]^{-1} 
\end{eqnarray}
...
\label{eqn:eddy_kt}
\end{equation}
Note that while we have not assumed a that $\varphi_u$ and
$\varphi_T$ are identical, for turbulent Prandtl numbers near unity 
(e.g. $Pr_T \approx 1$) they will be. The asymptotic behavior of
...
at the wall approaches zero ($q \to 0$). 
This is purely wind driven flow with no thermal differences. 
...
\end{equation}
Identical arguments can be made for the asymptotic behaviour of the temperature function.
...
does not account for the precence of the SoV device.\todo{entire section needs another read} 
...
The dimensionless constant $\bar \nu_V $ was calibrated against
laboratory generated experimental data 
(the table-top laboratory experiment is detailed in Section~\ref{sec:validation}), 
...
separation distance between neighboring vanes, or the radius of the SoV apparatus. 
The former is used for the unsteady virtual vanes, when greater spatial and 
temporal fidelity is required to capture the dynamics of the plume and wake. 
This viscosity is designed to represent the fluctuations inside the device 
due to separation of flow off the turning vanes, for instance. 
This is in contrast to the case of the steady virtual vanes, where 
the radius of the SoV apparatus is used as the length scale and 
no dynamics of the flow are are resolved. Here, the resolved scale is that of the
vortical plume itself, and not any fluctuating quantities. This is principally 
for design purposes, and is intented only to capture the largest scale 
features of the flow. These simulation regimes are discussed in further detail in 
...
We therefore represent the vanes as a force field, over 
which a force is applied to the velocity field to align it 
with the angle of the turning vanes, defined here as a 
field of vectors. 
These so-called `virtual vanes'' are implemented as a body force that 
is applied in the region that would otherwise contain the vanes. 
...
The quadratic functional form of this forcing can be motivated by 
the desire for a dimensionally consistent forcing. A linear form, 
for instance, is not dimensionally consistent. 
...
The length scale $\ell_v$ is a modeling constant and must be specified.\todo{this section needs another read}  
The length over which the flow comes into alignment 
with the vane direction is measured in Figure~\ref{fig:annealing}. 
This plots the average misfit between the turning
vane angle in an explicitly represented vane configuration and the flow. 
A value of one would represent flow that is perpendicular to the 
turning vanes, while a value of zero would represent perfectly aligned flow.
...
Notice that the misfit actually reaches a minimum and never comes into 
complete alignment with the vanes, likely due to unmodeled transient 
effects such as vortex shedding off the vane trailing edges. 
...
%channel. 
   ...
   \begin{figure}[!htb]
    ...
    logarithmic derivative of this quantity in black, and the average value of the
    logarithmic derivative in red.}
     \label{fig:annealing}
   \end{figure}

The value of $\ell_v$ can be calibrated to explicitly match the 
...
While the curve does not perfectly coincide
...
Where, based on results in the literature, $\beta = 2/3$ Kelvin per 
