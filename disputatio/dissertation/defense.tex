...
\frametitle{Project Objectives}
 ...
 \begin{block}{ARPA-E}
  \begin{itemize}
   ...
   \item Difficult to explore design space experimentally
	 \begin{itemize}
...
\frametitle{Project Objectives, cont.}
 ...
 \begin{itemize}
  ...
  \item Present Configuration and Results
 \end{itemize}
 \end{block}
...
\begin{frame}
 ...
 \begin{block}{Actuator Disk Modification}
   \begin{itemize}
    \small
    \item Actuator disk neglects interaction effects between turbine blades
%    \item Increase velocity seen by turbine blades to account for flow\
%	  going around the blades,

	  % \begin{equation}
	  %  \theta_f = \text{tan}^{-1}(\frac{{\bf
	  %   u_{\text{up}}}}{\textcolor{blue}{B(r)}\, {\bf
	  %   u_{\text{fwd}}}}).  
	  % \end{equation}
	  ...
	  %% \end{equation}
	  % Where $u_{\text{up}}$ and $u_{\text{fwd}}$ are the ``upward'' and
	  % ``forward'' velocities in the reference frame of the turbine 
	  % and $B(r)$ is a blockage factor (relative area) 

    \item Correction to forcing by reducing flow area,
	  \begin{equation}
	   B(r) = 1 - \frac{l_B(r)}{2 \pi r}
	  \end{equation}
	  Ratio: $l_B$ (length blocked by turbine blades) vs. total area
    \item As $l_B $ increases, velocity vector will align with drag	 
   \end{itemize}
 \end{block}
\end{frame}
