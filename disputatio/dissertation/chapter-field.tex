...
simulation was performed, and then the output was post-processed to evaluate the
...
the operation of the device.
 ...
 \begin{figure}[!htb]
  \begin{center}
   ...
   in Subsection~\ref{sec:interpolate}.} 
...
The cone plays at least two important roles. The first is acting 
much like a wind tunnel contraction, where the converging cone increases the
symmetry of the flow near the exit. This is important because the turbine 
can more easily extract power from a symmetric velocity field. 
...
%% $x_0,y_0,k,b$. 
%% This is, in principle, sufficient information to solve
...
%under-determined. 
%% The solution to the equation $A {\bf x} = {\bf b}$ is
%% found by computing a vector ${\bf x}$ that minimizes the Euclidean
%% 2-norm $|| {\bf b} - A {\bf x} ||^2$. 
...
polynomial remained accurate without requiring high order
...
manually integrating it through the field using the SciPy
\textbf{integrate.ode} packages and verifying visually that the vane had
the correct character. Figures~\ref{fig:top_design}
and~\ref{fig:bottom_design} were generated using this technique. 
...
of momentum flux. As both azimuthal and axial velocities depend on the
...
The UTRC frozen flow optimization was largely consistent with the fully
coupled CFD prediction (see  Figure~\ref{fig:UTRC_turbine}, below). This
has resulted in two final rotor designs, with and without twist. This
was because the experimental team was not certain that a twisted design 
...
this calculation, a temperature difference of 30 Kelvin was estimated.   
...
The wind direction was also monitored, and showed little variation in
heading. This is shown in Figure~\ref{fig:wind_direction}. It was
therefore assumed that the wind had a constant heading and constant
speed during the simulations. Furthermore, discussions with the field
team indicated that wind heading over several years of field tests were
relatively consistent. This was part of the motivation for the
introduction of an asymmetric vane configuration, since if the wind
direction changed daily (for instance) it would render an asymmetric
design useless, unless it could easily be realigned with the free stream
velocity.  
...
Simulations were performed to assess the variation in power extracted by
the turbine should the wind heading not align with the vanes.
%\todo{Todd asked, "Did you do any sensistivity studies to look at variation of performance of the optima%l design to the wind heading?}
It would be non-trivial to implement heading changes in the freestream velocity  
with the wind case boundary conditions discussed in
Section~\ref{sec:bc}. Instead, the vanes were rotated by $\pm
20^{\circ}$ and $\pm 45^{\circ}$ so that they did not align with the
freestream. %This was performed by creating a new interpolation vane
%function as described in Subsection~\ref{sec:interpolate} with the addition of 
%a linear rotation of the apparatus.

As the apparatus is not azimuthally symmetric, the vanes were
rotated both clockwise and counter-clockwise. A $20^{\circ}$
counter-clockwise adjustment reduced the power by
2\%, while a clockwise rotation resulted in a reduction of 8\%.
The apparatus is therefore likely not to be sensitive to directional heading  
changes of this magnitude. However, increasing the
rotation to 45 degrees clockwise reduces the power extracted by 63\% and
47\% for the counter-clockwise rotation. 
...
\begin{figure}[!htb]
  \centering
  \includegraphics[width=.32\linewidth]{figs/u_field_hor}
  \hfill
  \includegraphics[width=.32\linewidth]{figs/T_field_hor}
  \hfill
  \includegraphics[width=.32\linewidth]{figs/w_field_hor}
...
downwind side occurs near the surface. While most of the radial
...
% ratio of the free area in the disk versus the blade sizes.\todo{this is
% not the point}
...
This model modification indicates that the prior design of eight
...
Subsection~\ref{sec:field_error}.  
...
turning through the blades, and not separate. Thus, while the blockage
...
%Finally, the design space was limited to
%readily available drag polar data due to the expense and difficulty of 
%fabricating non-standard turbine blades. 
...
%likely not accurate for this case. Any analysis of the
...
coefficient, $C_D^{\text{Max}}$, we select a peak value of two. % refmunson 
