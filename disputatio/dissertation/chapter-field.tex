...
This now defines the normal vector.\todo{the rest of this section needs another look} 
However, we need to determine the 
constants (namely, $k,b$) that define ellipses that capture the shape we
desire. Further, we must determine an ellipse that has a certain
...
apparatus, and that the slope at this point is known. 
...
The challenge is to determine the the ellipse that
intersects a given point in space ($(x,y)$). 
This is solved discretizing the arc along the inner radius 
of the turning vanes, and generating data from
several curves that can be used to calibrate a 2D polynomial that will
smoothly vary in space. This is accomplish by choosing a series of
$x_0,y_0$ curves. Each unique ellipse is defined by ($x_0,y_0,b,a,k$),
...
\end{equation}
The x and y values of a particular ellipse, 
...
available power from the flow. This is an efficiency:\todo{another look here would be warranted} 
the power
...
with two or three blades). \todo{this section needs another look}
