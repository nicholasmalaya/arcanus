...
\end{equation}
From this point onward, the mesh has a uniform spacing, also of $h_{max}$, 
\begin{equation}
  h_{\text{max}} = \chi'(L_z) \, \hat h. 
\end{equation}

We further assume that the mapping has an exponential functional form, 
\begin{equation}
...
$C_1-C_3$ are scaling coefficients, and are defined as, %\todo{show coefficients}. 
\begin{align}
  C_1 &= C_2 \, C_3 e^{C_3 \, \hat z_b}, \\
  C_2 &= \frac{\hat z_b}{h_r -1}, \\
  C_3 &= \frac{\text{ln }(h_r) }{\hat z_b},
\end{align}
and $h_r$ is the ratio between the largest and smallest spacing, 
\begin{equation}
