...
\begin{align}
 ...
 (\nabla \cdot{\bf u},q) &= 0
 \label{eqn:cont_weak} \\
 (\frac{\partial T}{\partial t},{\bf w}) + ({\bf u} \cdot \nabla T,{\bf w}) + (k \, \nabla
 T, \nabla{\bf w}) &= 0.\label{eqn:en_weak}
\end{align}

$\forall ({\bf v},q,w) \in H^1(\Omega)^3 \times L_2(\Omega) \times
H^1(\Omega)$, where $(\cdot,\cdot)$ denotes the $L_2$ inner product, 
e.g. $(u,v) = \int_\Omega u \cdot v \, dx$ and $H^1(\Omega)$ is the
Sobolov space of with one square integrable derivative on the domain
$\Omega$\cite{}. As noted previously, boldface letters denote vector
quantities (such as ${\bf u} = \left \{ u,v,w \right \}$). 
