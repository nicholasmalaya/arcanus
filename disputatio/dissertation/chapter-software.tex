...
layer.\todo{the rest of this section needs another look}
...
The map $\chi$ spacing between points is initially a specified value, $h_{\text{min}}$, 
\begin{equation}
  h_{\text{min}} = \chi'(0) \hat h.
\end{equation}
Where $\hat h = L_z/\text{nz}$. The map is strictly increasing until the 
point $\hat z_b$, where the spacing reaches a constant, $h_{max}$, 
\begin{equation}
  h_{\text{max}} = \chi'(\hat z_b) \hat h. 
\end{equation}
After this point, the mesh has a uniform spacing of $h_{max}$, 
\begin{equation}
...
This mapping is defined by the following conditions.  The initial spacing is 
...
Where $z_b$ represents the chosen height of the 
boundary layer mesh, and is specified. 
