...
% for seeding are the center one (0.5” diameter) and the other 4 ones
% which are 0.25” diameter. The smallest holes are for alignment pins in
...
This quarter a new tool for rapidly investigating new design concepts
was developed. Previous results used a transient Navier-Stokes model,
and the solution was temporally averaged to capture the statistically
steady behavior of the flow. A new, Reduced Order Model (ROM) was
formulated in which the steady Navier Stokes equations are solved. This
has reduced the run-time from approximately 12 hours to less than two
minutes. This much reduced run-time permits rapid exploration of the SoV
configuration space, and may allow algorithmic optimization. The
solutions from the ROM are still undergoing validation against the
higher fidelity models, but across a range of previously tested
configurations the kinetic energy flux agrees within 15\% and the
sensitivities of the energy flux to small perturbations in the system
configuration correlate well with the results of the higher fidelity
transient model. It will be an invaluable design tool. A comparison of
the transient and reduced order steady models in shown in
Figure~\ref{}. The ROM is currently being used to explore new SoV vane
design concepts.  
...
\begin{figure}[htp!]
 ...
 \caption{The averaged transient virtual vane Solution (left image) is
 shown against the steady virtual vane solution (right image). While the
 steady solution is more diffuse, it possesses a similar qualitative
 structure as the higher fidelity solution.}   
...
%
% The averaged Transient Navier-Stokes Solution (left) is shown against
% the Reduced Order Steady Model (right). While the ROM solution is more
% diffuse, it possesses a similar qualitative structure as the higher
% fidelity solution. 
%
