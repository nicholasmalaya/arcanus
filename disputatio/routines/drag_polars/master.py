...
def drag_polar(name):
    ...
    sz=25
    plt.subplot(2, 1, 1)
    plt.plot(anglecd*np.pi, cd, 'ko-',label='COMSOL Data')
    plt.plot(np.pi*anglei, P.polyval(anglei,inter_cd), color='blue',label='Interpolant')
    ...
    plt.subplot(2, 1, 2)
    plt.plot(anglecl*np.pi, cl, 'ko-',label='COMSOL Data')
    plt.plot(anglei*np.pi, P.polyval(anglei,inter_cl), color='blue',label='Interpolant')
