...
This is a variation formulation that seeks to minimize the test function
$v$. Thus, any velocity that is not aligned with the vane normal will be
penalized. Note that unlike some penalty methods, this does not
automatically satisfy continuity. Rather, the velocity field remains
divergence free through a separately enforced constraint. 
...
\begin{center}
 \begin{algorithm}
  \caption{Separation model}
  \label{alg:sep}
  \begin{algorithmic}[1]
   \If{($r_0 > r > r_i$)}
   \If{$(r_0 - r) < \delta$ \textbf{or} $((r - r_i) < \delta)$}
   \State $\overrightarrow{n_r} = \overrightarrow{r}/|r|$
   \If{$(r - r_i) < \delta$} 
   \State $\overrightarrow{n_r} = -\overrightarrow{n_r}$
   \EndIf
   \State $\overrightarrow{t_v} = (n_y^v , -n_x^v) \text{ sign
