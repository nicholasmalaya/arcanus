...
three system diameters ($L_z/D = 3$). Thus, the wind domain $\Omega_W$ is 
...
Where $\Gamma_G$ corresponds to the boundary along the ``Ground'', $\Gamma_T$ the ``Top'' boundary, 
$\Gamma_S$ the two ``Sides'', $\Gamma_I$ the inflow boundary, and $\Gamma_O$ the outflow boundary.
...
A periodic boundary condition $\Gamma_P$ is used in the thermal only cases, 
along the streamwise and spanwise boundary faces. In these cases the state variables 
are made to agree with the value at the opposite face, for instance the 
streamwise boundary will have the form, 
\begin{align}
 \overrightarrow{u}(-L_x,y,z) &= \overrightarrow{u}(L_x,y,z) \quad \text{ on } \Gamma_P \\
 T(-L_x,y,z) &= T(L_x,y,z). 
\end{align}
And the spanwise boundary will enforce, 
\begin{align}
 \overrightarrow{u}(x,-L_y,z) &= \overrightarrow{u}(x,L_y,z) \quad \text{ on } \Gamma_P \\
 T(x,-L_y,z) &= T(x,L_y,z). 
\end{align}
...
Thus, the top boundary condition $\Gamma_T$, has the form, 
\begin{align}
  \frac{\partial u}{\partial n}\bigg|_{\Gamma_T} = 0 \\
  blah
\end{align}

Where\todo{fix eqn above} 
the top boundary condition is defined to lie at the furthest vertical extent of the domain: 
$\Gamma_T = \{(x,y,L_z) \subset \partial \Omega \} $. 
To ensure the problem is well-posed, the ``v'' and ``u'' components of
inflow velocity are set to zero for the thermal-only, and $v=0$, 
$u=U$ for the wind cases. 

Nearly identical conditions are imposed on the sides and back boundaries in the wind case. 
...
Likewise, the back boundary condition ($\Gamma_B$) is tested to ensure that the streamwise 
