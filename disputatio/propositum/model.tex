...
radiation is absorbed by the ground, which causes it's temperature to
...
provides guidance on more general character of the functions. We are
interested in the case where $L_{M-O}<0$, which corresponds to heat flux
...
extents ($L_x/D$,($L_y/D$) are chosen to be 3. The height ($L_z/D$) is
three times the system diameter, which is typically nearly equal to the
height of the vanes. This defines the domain $\Omega$, for instance the
thermal-only domain is $\Omega_T = \left[-L_x,L_x \right] \times
...
wind domain is $\Omega_W = \left[-2D,3D \right] \times \left[-L_y,L_y
\right] \times \left[0,L_z \right]$.  
...
\begin{equation*}
  T_{in}(z) = \Delta T \left(1- \text{ min
