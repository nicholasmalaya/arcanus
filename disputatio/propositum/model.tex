...
faces. The domain extents are scaled by system diameter. 
...
ground).\todo{write down the math} On the ground, a ``no-slip'' velocity
boundary condition is imposed on the velocity field, and a dirichlet
condition uniformly fixes the temperature of the surface. These boundary
conditions are mathematically defined in the subsequent labeled
paragraphs. 
...
\begin{figure}[!htb]
  \begin{center}
   ...
   precise boundary conditions on each provided in section~\ref{sec:bc}.} 
    \label{fig:thermal3d}
  \end{center}
\end{figure}
