...
faces. The domain extents are scaled by system diameter, (D). For the
thermal-only, which is identical in $L_x$ and $L_y$, the system extents
($L_x/D$,($L_y/D$) are chosen to be 6. The height ($L_z/D$) is three
times the system diameter, which is typically nearly equal to the height
of the vanes.  

For the wind cases, the streamwise extent, $L_x$, is no longer equal to
the spanwise length, $L_y$. In these cases, the domain length extends
two diameters in front of the vanes and three behind ($L_x/D = 5$). The
spanwise direction extends two diameters in each direct from the center
($L_y/D = 4$). 

A 3d diagram of the ``thermal-only'' simulation is shown in
...
top boundary, and Dirichlet boundary conditions on the bottom (the
ground). On the ground, a ``no-slip'' velocity boundary condition is
imposed on the velocity field, and a Dirichlet condition uniformly fixes
the temperature of the surface. 
These boundary conditions are mathematically expressed as a boundary
$\partial \Omega = $ a domain
$\Omega \subset \mathbb{R}^3$ 

These boundary conditions are
mathematically defined in the subsequent labeled paragraphs. 
...
A 3d diagram of the ``wind'' simulation is diagrammed in
Figure~\ref{fig:wind3d}. For this particular study (a heated ground with 
...
top are all set to modified neumann boundary conditions, as described
above. The outflow region in the back also needs the modified neumann as
it does occasionally exhibit mild inflow on account of the unsteady wake. 
...
\textbf{Ground Boundary Conditions} 
...
%
%
\textbf{Side Boundary Conditions} 
...
The velocity boundary layer is set to the common 7th power function for a
...
selected\cite{Blocken2007238}. 

\textbf{Mixed inflow/outflow Boundary Conditions} 

A modified neumann condition is necessary due to the presence of
both\todo{write the math} inflow and outflow across the face. In the
region approximately above the vanes, the concentrated hot plume is
...
vanes. To avoid an ill-posed problem, the ``v'' and ``u'' components of
inflow velocity are set to zero.

\textbf{Sponge Layer} 

Finally, a finite thickness ``sponge layer'' is used in front of the
mixed inflow/outflow boundaries. 
