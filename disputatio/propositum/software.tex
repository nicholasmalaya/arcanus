...
mention what happens when steady

Using linear basis functions for both the velocity and pressure. 
This scheme circumvents
the Babuska-Brezzi conditions\cite{bb-cond} and uses equal-order
elements for velocity and  pressure, by introducing a streamline
upwind/Petrov-Galerkin (SUPG) stabilization term as first described by
Hughes\cite{Hughes198685,supg} and extended to natural convection as in
Becker and Braack\cite{Becker2002428}. The stabilization of the
underlying Navier-Stokes equations is accomplished by the introduction
of additional terms ($\tau()$) that have the flavor\todo{indicate stabiliziation used}
